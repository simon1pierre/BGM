    ...
    <section class="pt-16 pb-10 bg-gradient-to-b from-blue-950 to-slate-900 text-white">
        <div class="container mx-auto px-6">
            <div class="max-w-4xl">
                ...
                <p class="text-blue-100/90 text-lg">{{ $audio->category?->name ?? 'Audio' }}</p>
            </div>
        </div>
    </section>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <div class="text-xs uppercase tracking-widest text-slate-500 mb-2">Now Playing</div>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <div class="flex items-center gap-4 text-sm text-slate-600 mb-4">
                            ...
                            >
                                Comment (<span data-comment-count>{{ $audio->comments_count ?? 0 }}</span>)
                            </button>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <h2 class="text-xl font-serif font-bold text-blue-950 mb-3">About this audio</h2>
                        <p class="text-slate-600 text-sm leading-relaxed">{{ $audio->description }}</p>
                        <div class="mt-4 text-xs text-slate-500">
                            Published: {{ $audio->published_at?->toDateString() ?? $audio->created_at?->toDateString() }}
                        </div>
                    </div>
                </div>
                <div class="space-y-6">
            ...
            @if (!empty($relatedAudios) && $relatedAudios->count())
                <div class="mt-12">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-xl font-serif font-bold text-blue-950">More Like This</h3>
                        <a href="{{ route('audios.index') }}" class="text-sm text-blue-700 hover:text-blue-900">Browse all</a>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        @foreach ($relatedAudios as $item)
                            <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100">
                                ...
                                <div class="p-4">
                                    <div class="text-xs text-slate-500">{{ $item->category?->name ?? 'Audio' }}</div>
                                    <a href="{{ route('audios.show', $item) }}" class="inline-flex text-sm text-blue-700 hover:text-blue-900 mt-3">Open</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
